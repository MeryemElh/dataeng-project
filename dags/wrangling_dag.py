...
def _saving_to_neo4j():
    query = "SELECT artist_id, target_id, a.name AS artist_name, b.name AS target_name FROM song, entity as a, entity as b WHERE artist_id=a.id AND target_id=b.id"
    engine = create_engine(
        f'postgresql://{"airflow"}:{"airflow"}@{"postgres"}:{"5432"}/{"postgres"}'
    )
